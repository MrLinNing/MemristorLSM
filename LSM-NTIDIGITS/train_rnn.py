...
from models import RNN_ModelFull
...
def parse_args():
    ...
    ## hyper-parameters
    parser.add_argument('--r_noise', type=float, default=0.0)
    ...
    parser.add_argument('--ts', type=int, default=50, metavar='T',
                        help='time_step')
    parser.add_argument('--Tw', type=int, default=30, metavar='T',
                        help='Time window value of LIF')
    parser.add_argument('--ARCHI', type=str, default="64,200,11", metavar='A',
...
SAVE_DIR = os.path.join(SAVE_DIR,
                        f'b{options.batch}_lr{options.lr}_ARCHI{cfg_net}_dt{options.Dt}_{t}')
...
'''Model Loading'''
net = RNN_ModelFull(cfg=cfg_net, r_noise=0)
net.to(device)
...
for epoch in range(options.epochs):
    ...
    for i, (images, labels) in enumerate(train_loader_crop):
        ...
        outputs, _ = net(images0, time_window)
