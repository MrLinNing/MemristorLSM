...
from models import SRNN_SNN_Model
...
def parse_args():
    ...
    parser.add_argument('--Decay', type=float, default=0.9,
                        help='Decay Value of snn-type readout')
...
SAVE_DIR = os.path.join(SAVE_DIR,
                        f'b{options.batch}_timewindow{options.Tw}_decaylsm{options.Decay_lsm}_decay{options.Decay}_lr{options.lr}_ARCHI{cfg_net}_Vth{options.Vth}_const{options.const}_scale{options.scale}_{t}')
...
net = SRNN_SNN_Model(cfg=cfg_net, time_window=options.Tw, decay=options.Decay, lens=0.25,
                         thresh=options.Vth, decay_lsm=options.Decay_lsm, const=options.const,
                         scale=options.scale)
net.to(device)
...
for epoch in range(options.epochs):
    ...
    for images, labels in test_loader_crop:
        ...
        #         images1 = images[:,1,:,:,:].unsqueeze(1).float().to(device)
        outputs = net(images0)
